...
#
#   KEY       = paste0(SUBJID, "_", VSTESTCD) # nolint: commented_code_linter.
...
add_display_columns <- function(data, arg_grp = "TRTA", page_size = SUBJECT_PAGE_SIZE) { # nolint
  # nolint: trailing_whitespace_linter.
  # Build SUBJID → SUBJ_PAGE mapping within each treatment group
  page_map <- data |>
    dplyr::select(SUBJID, !!rlang::sym(arg_grp)) |> # nolint
    dplyr::distinct() |>
    dplyr::arrange(!!rlang::sym(arg_grp), SUBJID) |>
    dplyr::group_by(!!rlang::sym(arg_grp)) |>
    dplyr::mutate(
      row_in_grp = dplyr::row_number(),
      page_num   = ceiling(row_in_grp / page_size) # nolint: object_usage_linter, line_length_linter.
    ) |>
    dplyr::group_by(!!rlang::sym(arg_grp), page_num) |> # nolint: object_usage_linter, line_length_linter.
    dplyr::mutate(
            SUBJ_PAGE = page_num # nolint: indentation_linter.
      #SUBJ_PAGE = paste0(dplyr::first(row_in_grp), "\u2013", dplyr::last(row_in_grp)) # nolint
    ) |>
    dplyr::ungroup() |>
    dplyr::select(SUBJID, !!rlang::sym(arg_grp), SUBJ_PAGE) # nolint
  # nolint
  data |>
    dplyr::mutate(KEY = paste0(SUBJID, "_", VSTESTCD)) |> # nolint
...
#' Build a per-treatment SUBJ_PAGE → KEY-vector mapping for the JavaScript filter # nolint
#'
...
#'
#' @param data  Data frame after add_display_columns() (needs TRTA, SUBJ_PAGE, KEY). # nolint
#' @return Named list: treatment → (page number as string → KEY character vector). # nolint
build_subj_page_map <- function(data) {
  ...
  stats::setNames(
    lapply(trts, function(trt) {
      ...
      stats::setNames(
        lapply(pages, function(pg) unique(trt_data$KEY[trt_data$SUBJ_PAGE == pg])), # nolint
        as.character(pages)
      )
    }),
    trts
